...
def predict():
    ...
    try:
        ...
        required_columns = ['PhoneService', 'MultipleLines', 'InternetService', 'OnlineSecurity', 
                            'OnlineBackup', 'DeviceProtection', 'TechSupport', 'StreamingTV', 'StreamingMovies']
        missing_columns = [col for col in required_columns if col not in df.columns]
        
        if missing_columns:
            logging.error(f"❌ Colonnes manquantes dans les données d'entrée : {missing_columns}")
            return jsonify({"error": "Colonnes manquantes dans les données d'entrée", "missing_columns": missing_columns}), 400
        
        # Prétraitement des données
        df = clean_data(df)
        logging.info(f"🔹 Données après nettoyage :\n{df.head()}")
        
        df = remove_multicollinearity(df)
        logging.info(f"🔹 Données après suppression de la multicolinéarité :\n{df.head()}")
        
        df = normalize_features(df)
        logging.info(f"🔹 Données après normalisation :\n{df.head()}")
        ...
        if missing_for_model:
            logging.error(f"❌ Colonnes manquantes après prétraitement : {missing_for_model}")
